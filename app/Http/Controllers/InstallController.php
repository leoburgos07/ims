...
Class InstallController extends Controller{
	...
	public function store(InstallRequest $request){
		$url = "http://verify.wmlab.in/index.php?envato_username=".$request->input('envato_username')."&purchase_code=".$request->input('purchase_code')."&product=13760351";
				...
				return redirect()->back()->withInput()->withErrors('Connection could not be established!!');
					...
					if (!is_file('../database/database.sql'))
						return redirect()->back()->withInput()->withErrors('Database file not found!!');
							...
							mysqli_query($link, "insert into roles(name,display_name) values('admin','Admin'),('user','User') ");
